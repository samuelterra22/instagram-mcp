#!/usr/bin/env python3
"""Dirige o MCP server via stdio, chama as tools de LEITURA e reporta o que funciona.
Tools de escrita (publish/send_dm/comment/delete/hide) NAO sao executadas: risco
irreversivel/publico na conta real. Sao listadas como capacidade.
"""
...
def data_list(payload):
    """Extrai lista de itens de formatos variados: [...], {data:[...]},
...
print("=== LEITURA (executadas ao vivo) ===")
...
# media posts -> pega 1 media_id
ok, txt, p = call("get_media_posts", {"limit": 5})
media_id = extract_id(p)
rec("get_media_posts", ok, f"1o id={media_id} | " + summarize(p, []))
...
if media_id:
    ...
    rec("get_media_insights", False, "sem media_id (nenhum post) -> pulado")
    rec("get_comments", False, "sem media_id -> pulado")
...
if conv_id:
    ...
    rec("get_conversation_messages", False, "sem conversa -> pulado")
...
print(f"\n=== RESUMO LEITURA: {okc}/{len(results)} OK ===")
